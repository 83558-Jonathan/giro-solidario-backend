...
const VALOR_BASE = 125;
const VALOR_COM_TAXA = VALOR_BASE * (1 + TAXA_PLATAFORMA); // 137.50
...
exports.criarCobrancaPix = async (req, res) => {
    try {
        ...
        // ===========================================
        // GARANTIR VALOR COM TAXA (10%)
        // ===========================================
        const valorCorreto = VALOR_COM_TAXA; // 137.50
        const valorCentavos = Math.round(valorCorreto * 100); // 13750
        ...
        console.log('📥 Resposta da AbacatePay:', JSON.stringify(response.data, null, 2));
        ...
        transacao.valorPago = valorCorreto; // 137.50
        transacao.metadata = {
            ...(transacao.metadata || {}),
        ...
        console.log(`💰 PIX gerado para transação ${transacaoId}: R$ ${valorCorreto}`);
        ...
        res.status(500).json({
            success: false,
            error: error.response?.data?.error || 'Erro ao gerar PIX. Tente novamente.'
        });
    }
};
    ...
    try {
        ...
        if (transacao.cobrancaId) {
            try {
                ...
                const statusApi = response.data.data?.status?.toUpperCase?.() || response.data.data?.status;
...
exports.renovarCobrancaPix = async (req, res) => {
    try {
        ...
        // ===========================================
        // GARANTIR VALOR COM TAXA (10%) - MESMO NA RENOVAÇÃO
        // ===========================================
        const valorCorreto = VALOR_COM_TAXA; // 137.50
        const valorCentavos = Math.round(valorCorreto * 100); // 13750
        ...
        const response = await abacate.post('/pixQrCode/create', payload);
        const { id: novaCobrancaId, brCode, brCodeBase64, expiresAt } = response.data.data;
        ...
        console.log(`💰 PIX renovado para transação ${transacaoId} (renovação #${renovacoes}) - R$ ${valorCorreto}`);
