...
exports.minhasTransacoes = async (req, res) => {
  try {
    ...
    res.status(500).json({
      success: false,
      error: process.env.NODE_ENV === 'development' ? error.message : 'Erro ao buscar transações'
    });
  }
};
...
exports.porRodada = async (req, res) => {
  try {
    ...
    res.status(500).json({
      success: false,
      error: process.env.NODE_ENV === 'development' ? error.message : 'Erro ao listar transações'
    });
  }
};
...
exports.confirmarDeposito = async (req, res) => {
  try {
    ...
    res.status(500).json({
      success: false,
      error: process.env.NODE_ENV === 'development' ? error.message : 'Erro ao confirmar depósito'
    });
  }
};
...
exports.cancelarTransacao = async (req, res) => {
  try {
    ...
    res.status(500).json({
      success: false,
      error: process.env.NODE_ENV === 'development' ? error.message : 'Erro ao cancelar transação'
    });
  }
};
...
exports.estatisticas = async (req, res) => {
  try {
    ...
    const [totalPendente, totalConfirmado, totalComoPagador, totalComoRecebedor] = await Promise.all([
    ...
    res.status(500).json({
      success: false,
      error: process.env.NODE_ENV === 'development' ? error.message : 'Erro ao buscar estatísticas'
    });
  }
};