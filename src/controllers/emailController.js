            ...
            <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
                ...
                <div style="padding: 30px; background-color: #f9fafb; border: 1px solid #e5e7eb; border-radius: 0 0 10px 10px;">
                    ...
                    <p style="color: #9ca3af; font-size: 12px; text-align: center;">
                        ...
                        }" style="color: #9ca3af;">Acessar Sistema</a>
                    </p>
                </div>
            </div>
        `
...
async function enviarEmailCobranca (
  ...
  if (!smtpOk) {
    ...
    console.log(
      `   Assunto: ${
        tipo === 'lembrete' ? 'Lembrete de pagamento' : 'Pagamento pendente'
      } - ${rodada.nome}`
    )
    console.log(`   Corpo:`)
    console.log(`   Olá ${usuario.nome}!`)
    console.log(
      `   Seu pagamento de R$ ${valor.toFixed(2)} para a rodada ${
        rodada.nome
      } está pendente.`
    )
    ...
    if (!podeEnviarEmail(usuarioId, rodadaId)) {
      ...
      return res.status(429).json({
        ...
        error: `Aguarde ${Math.ceil(
          horasRestantes
        )} horas para enviar outro lembrete`,
...
exports.enviarLembrete = async (req, res) => {
  try {
    ...
    if (!podeEnviarEmail(usuarioId, rodadaId)) {
      ...
      return res.status(429).json({
        success: false,
        error: `Já enviamos um lembrete recentemente. Aguarde ${Math.ceil(
          horasRestantes
        )} horas.`,
        podeEnviarNovamente: false,
        horasRestantes: Math.ceil(horasRestantes)
    ...
    const linkPagamento = transacao
      ? `${process.env.FRONTEND_URL || 'http://localhost:3000'}/pagamento/${
          transacao._id
        }`
      : null
    ...
    html: `
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
        ...
        <div style="padding: 30px; background-color: #f9fafb;">
          ...
          <p><strong>Chave PIX cadastrada:</strong> ${usuario.chavePix} (${
...
async function enviarEmailQrCodePix (
  ...
  // Garante que a imagem base64 tenha o prefixo correto
  const imageSrc =
      ...
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
        ...
        <div style="padding: 30px; background-color: #f9fafb; border: 1px solid #e5e7eb; border-radius: 0 0 10px 10px;">
          ...
          <div style="background-color: #fef3c7; padding: 20px; border-radius: 12px; margin: 20px 0; text-align: center;">
            ...
            <p style="margin: 5px 0 0; font-size: 12px; color: #92400e;">(Investimento de R$ ${valor.toFixed(
              2
            )})</p>
          </div>
          ...
          <div style="background-color: white; padding: 20px; border-radius: 12px; margin: 20px 0; text-align: center; border: 2px solid #e5e7eb;">
            ...
            ${
              !qrCodeImage && qrCode
                ? `<p style="font-size: 12px; word-break: break-all; background: #f3f4f6; padding: 10px; border-radius: 8px;">${qrCode}</p>`
                : ''
            }
          </div>
